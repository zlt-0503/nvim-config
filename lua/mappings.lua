...
NvimConfig = NvimConfig or {}

function NvimConfig.has_makefile()
  return vim.fn.filereadable(vim.fn.getcwd() .. "/Makefile") == 1
end

function NvimConfig.make_or_run(fallback)
  if NvimConfig.has_makefile() then
    vim.cmd("update")
    vim.cmd("make")
    return
  end
  ...
  if type(fallback) == "function" then
    ...
    vim.notify("No Makefile found and no <F9> fallback is configured for this filetype.", vim.log.levels.WARN, {
      title = "nvim-config",
    })
  end
end

function NvimConfig.make_or_cmd(cmd)
  NvimConfig.make_or_run(function()
    vim.cmd(cmd)
  end)
end

function NvimConfig.make_or_keys(keys)
  NvimConfig.make_or_run(function()
...
function NvimConfig.json_format(line1, line2)
  ...
  local function encode_pretty(value, indent)
    ...
    if is_array(value) then
      ...
      for _, item in ipairs(value) do
        table.insert(chunks, child_pad .. encode_pretty(item, indent + 1))
      end
      return "[\n" .. table.concat(chunks, ",\n") .. "\n" .. pad .. "]"
    end
    ...
    for _, key in ipairs(keys) do
      table.insert(chunks, child_pad .. vim.json.encode(key) .. ": " .. encode_pretty(value[key], indent + 1))
    end
    return "{\n" .. table.concat(chunks, ",\n") .. "\n" .. pad .. "}"
  end
  ...
  if not ok then
    vim.notify("Invalid JSON: " .. decoded, vim.log.levels.ERROR, { title = "nvim-config" })
    return
  end

  local encoded = encode_pretty(decoded)
  local formatted = vim.split(encoded, "\n", { plain = true })
  api.nvim_buf_set_lines(0, line1 - 1, line2, false, formatted)
end
...
keymap.set("n", "<F9>", function()
  NvimConfig.make_or_run()
end, { desc = "run make from cwd" })
